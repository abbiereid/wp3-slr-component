            ...
            for sample_index, filename in enumerate(filenames):
                ...
                if args.embedding_kind != "probabilities":
                    if args.embedding_kind == "CLS":
                        output = hook_outputs[0][sample_index]  # CLS is not time based anymore, so first axis is batch.
                    else:
                        output = hook_outputs[0][:,
                                 sample_index]  # Spatial and temporal are time based, first axis is time.
                        output = output[0:batch.lengths[sample_index]]  # Unpad.
